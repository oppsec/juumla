...
def file_scan(file_name: str, file_list: dict, file_url: str) -> None:
    for file_name in file_list:
        file_final_url = f"{file_url}/{file_name}"

        try:
            response = get(file_final_url, **props)

            if response.ok:
                print(f"[green]> Sensitive file found: {file_final_url} [/]")
            else:
                pass
            
        except Exception as error:
            return print(f"[red]> Error when trying to find sensitive files: {error} [/]")
...
def files_manager(url) -> None:
    ...
    for file in config_files:
        file_scan(file, config_files, url)

    # Scan for generic backup files
    for file in bkp_files:
        file_scan(file, bkp_files, url)
